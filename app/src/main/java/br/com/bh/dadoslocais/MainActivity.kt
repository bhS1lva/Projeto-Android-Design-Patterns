...
import br.com.bh.dadoslocais.database.repository.ContatoRepository
import br.com.bh.dadoslocais.model.Contato
import br.com.bh.dadoslocais.ui.theme.DadosLocaisTheme
...
fun ContatosScreen() {
    ...
    val contatoRepository = ContatoRepository(LocalContext.current)

    val listaContatosState = remember {
        mutableStateOf(contatoRepository.listarContatos())
    }

    val atualizarListaContatos: () -> Unit = {
    ...
    Column {
        ContatoForm(
            ...
            atualizar = atualizarListaContatos
        )
        ContatoList(
            listaContatosState,
            atualizar = atualizarListaContatos
...
@Composable
fun ContatoList(listaContatosState: MutableState<List<Contato>>, atualizar: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
        ...
        for (contato in listaContatosState.value) {
